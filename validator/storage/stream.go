...
func NewStorageStream(toAddr common.Address, statedb vm.StateDB) *StorageStream {
...
func (s *StorageStream) Flush() {
	for k, v := range s.bufferedSlots {
		s.stateDb.SetState(s.toAddress, k, common.Hash(*v))
	}
}
...
func (s *StorageStream) getSlot(off *big.Int) error {
	...
	if !ok {
		tmp := Slot(s.stateDb.GetState(s.toAddress, slotKey))
		slot = &tmp
