...
func NewSimulatedBackendWithDatabase(database ethdb.Database, alloc core.GenesisAlloc, gasLimit uint64) *SimulatedBackend {
	...
	var genesisEraLength uint64
	if genesis.Config.FixEraLengthNumber == 0 {
		genesisEraLength = genesis.Config.EpochsPerEra
	} else {
...
func (m callMsg) SetFake(isFake bool) types.Message { panic("implement me") }
...
func (fb *filterBackend) GetReceipts(ctx context.Context, hash common.Hash) (types.Receipts, error) {
	number := rawdb.ReadFinalizedNumberByHash(fb.db, hash)
	if number == nil {
		return nil, nil
	}
	return rawdb.ReadReceipts(fb.db, hash, fb.bc.Config()), nil
}
...
func (fb *filterBackend) GetLogs(ctx context.Context, hash common.Hash) ([][]*types.Log, error) {
	...
	receipts := rawdb.ReadReceipts(fb.db, hash, fb.bc.Config())
	if receipts == nil {
		return nil, nil
	}
