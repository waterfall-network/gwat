...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "Waterfall app in browser mode", w.failure
	}
	if w.offline() {
		return "Waterfall app offline", w.failure
	}
	return fmt.Sprintf("Waterfall app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}
