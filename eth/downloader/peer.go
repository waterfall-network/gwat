...
import (
	...
	"gitlab.waterfall.network/waterfall/protocol/gwat/p2p/msgrate"
)
...
type Peer interface {
	...
	RequestDag(uint64, uint64) error
}
...
func (w *lightPeerWrapper) RequestDag(fromCpEpoch uint64, toCpEpoch uint64) error {
...
func (p *peerConnection) FetchDag(fromCpEpoch, toCpEpoch uint64) error {
	// Short circuit if the peer is already fetching
