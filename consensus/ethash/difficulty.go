...
func CalcDifficultyFrontierU256(time uint64, parent *types.Header) *big.Int {
	...
	*/
	...
	pDiff, _ := uint256.FromBig(big1) // pDiff: pdiff
	adjust := pDiff.Clone()
	...
	if periodCount := (parent.Height + 1) / expDiffPeriodUint; periodCount > 1 {
...
func CalcDifficultyHomesteadU256(time uint64, parent *types.Header) *big.Int {
	...
	*/
	...
	// diff = diff + 2^(periodCount - 2)
	if periodCount := (1 + parent.Height) / expDiffPeriodUint; periodCount > 1 {
