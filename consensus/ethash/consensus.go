...
func (ethash *Ethash) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, fulldag bool) error {
	...
	if fulldag {
		...
		if dataset.generated() {
			digest, result = hashimotoFull(dataset.dataset, ethash.SealHash(header).Bytes(), uint64(0))
	...
	if !fulldag {
		...
		digest, result = hashimotoLight(size, cache.cache, ethash.SealHash(header).Bytes(), uint64(0))
