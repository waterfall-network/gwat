...
import (
	...
	"gitlab.waterfall.network/waterfall/protocol/gwat/crypto/signify"
	"gitlab.waterfall.network/waterfall/protocol/gwat/internal/build"
...
var (
	...
	debExecutables = []debExecutable{
		{
			BinaryName:  "abigen",
			Description: "Source code generator to convert Waterfall contract definitions into easy to use, compile-time type-safe Go packages.",
		},
		{
			BinaryName:  "bootnode",
		...
		{
			BinaryName:  "puppeth",
			Description: "Waterfall private network manager.",
		},
		{
			BinaryName:  "rlpdump",
		...
		{
			BinaryName:  "clef",
			Description: "Waterfall account management tool.",
		},
	}
...
func doDocker(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the docker image (usually "waterfall/client-go")`)
	)
	flag.CommandLine.Parse(cmdline)
...
func doDebianSource(cmdline []string) {
	var (
		...
		upload   = flag.String("upload", "", `Where to upload the source package (usually "waterfall/waterfall")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.TempDir("", "geth-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "gitlab.waterfall.network/waterfall/protocol/gwat/mobile"))
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "gitlab.waterfall.network/waterfall/protocol/gwat/mobile")
