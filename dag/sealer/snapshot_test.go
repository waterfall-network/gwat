...
// the account may or may not have cast a Sealer vote.
...
// Tests that Sealer signer voting is evaluated correctly for various simple and
...
func TestClique(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for j, block := range blocks {
			...
			header := block.Header()
			if j > 0 {
				header.ParentHashes[0] = blocks[j-1].Hash()
			}
		...
		snap, err := engine.snapshot(chain, 0, head.Hash(), nil)
		if err != nil {
